...
d3.json("data/stations_all.json").then(function(stationsData) {
    ...
    function updateTable(selectedStation) {
        ...
        // Create list of rows to iterate through
        ...
        header.text(`Station ID: ${selectedData.usaf}, Station Name: ${selectedData.name}, State: ${selectedData.state}`)
        
        // Iterate through the data ids to update the table
        for (i=1; i < tableIDs.length; i++){
            let cell = d3.select("#" + tableIDs[i])
            cell.text(selectedData[tableIDs[i]])
        }
        // tbody.append("tr")
    ...
    stationsData.forEach(station => {
        stationSelect.append("option")
            ...
            .text(`${station.state}: ${station.usaf} - ${station.name}`); // Bekah wants sort by state
